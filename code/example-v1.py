...
def Check_4_in_line(new_board, move, side):  # move 表示检查那个点附近 side表示检查哪一边的，num表示检查连续几个
    ...
    for i in range(min_x + 1, max_x + 1):  # 竖着4个
        ...
        if count == 2:  # 检查 xx xx 的情况
            x1, y1 = end_pos
            if x1 + 3 <= MAX_BOARD - 1 and new_board[x1 + 1][y1] == 0 and new_board[x1 + 2][y1] == side and \
                    new_board[x1 + 3][y1] == side:
                force_move.append((x1 + 1, y1))
            if x1 - 4 >= 0 and new_board[x1 - 2][y1] == 0 and new_board[x1 - 3][y1] == side and new_board[x1 - 4][
                y1] == side:
    ...
    for i in range(min_y + 1, max_y + 1):  # 横着的4个
        ...
        if count == 2:  # 检查 xx xx 的情况
            x1, y1 = end_pos
            if y1 + 3 <= MAX_BOARD - 1 and new_board[x1][y1 + 1] == 0 and new_board[x1][y1 + 2] == side and \
                    new_board[x1][y1 + 3] == side:
                force_move.append((x1, y1 + 1))
            if y1 - 4 >= 0 and new_board[x1][y1 - 2] == 0 and new_board[x1][y1 - 3] == side and new_board[x1][
                y1 - 4] == side:
    ...
    for m in range(-down, up):  # 正斜的时候
        ...
        if count == 3:  # 检查xxx x 或者x xxx的情况
            x1, y1 = end_pos
            if x1 + 2 <= MAX_BOARD - 1 and y1 + 2 <= MAX_BOARD - 1 and new_board[x1 + 1][y1 + 1] == 0 and \
                    new_board[x1 + 2][y1 + 2] == side:
                force_move.append((x1 + 1, y1 + 1))
            if x1 - num - 1 >= 0 and y1 - num - 1 >= 0 and new_board[x1 - num][y1 - num] == 0 and \
                    new_board[x1 - num - 1][y1 - num - 1] == side:
        ...
        if count == 2:  # 检查 xx xx 的情况
            x1, y1 = end_pos
            if x1 + 3 <= MAX_BOARD - 1 and y1 + 3 <= MAX_BOARD - 1 and new_board[x1 + 1][y1 + 1] == 0 and \
                    new_board[x1 + 2][y1 + 2] == side and new_board[x1 + 3][y1 + 3] == side:
                force_move.append((x1 + 1, y1 + 1))
            if x1 - 4 >= 0 and y1 - 4 >= 0 and new_board[x1 - 2][y1 - 2] == 0 and new_board[x1 - 3][y1 - 3] == side and \
                    new_board[x1 - 4][y1 - 4] == side:
    ...
    for m in range(-down, up):  # 检查侧斜的情况
        ...
        if count == num + 1:  # 检查xxx的情况 num = 2 count = 3
            ...
            if x1 + 1 <= MAX_BOARD - 1 and y1 - 1 >= 0 and new_board[x1 + 1][y1 - 1] == 0:  # 往后一格看看行不行
                force_move.append((x1 + 1, y1 - 1))
            if x1 - num - 1 >= 0 and y1 + num + 1 <= MAX_BOARD - 1 and new_board[x1 - num - 1][
                y1 + num + 1] == 0:  # 往前看一步行不行
                force_move.append((x1 - num - 1, y1 + num + 1))
            if len(force_move) != 0:
                force_move_all.append(force_move)
        ...
        if count == num:  # 检查xxx x 或者x xxx的情况 num = 2 count = 2
            x1, y1 = end_pos
            if x1 + 2 <= MAX_BOARD - 1 and y1 - 2 >= 0 and new_board[x1 + 1][y1 - 1] == 0 and new_board[x1 + 2][
                y1 - 2] == side:
                force_move.append((x1 + 1, y1 - 1))
            if x1 - num - 1 >= 0 and y1 + num + 1 <= MAX_BOARD - 1 and new_board[x1 - num][y1 + num] == 0 and \
                    new_board[x1 - num - 1][y1 + num + 1] == side:
                force_move.append((x1 - num, y1 + num))
            if len(force_move) != 0:
                force_move_all.append(force_move)
        ...
        if count == 2:  # 检查 xx xx 的情况
            x1, y1 = end_pos
            if x1 + 3 <= MAX_BOARD - 1 and y1 - 3 >= 0 and new_board[x1 + 1][y1 - 1] == 0 and new_board[x1 + 2][
                y1 - 2] == side and new_board[x1 + 3][y1 - 3] == side:
                force_move.append((x1 + 1, y1 - 1))
            if x1 - 4 >= 0 and y1 + 4 <= MAX_BOARD - 1 and new_board[x1 - 2][y1 + 2] == 0 and new_board[x1 - 3][
                y1 + 3] == side and new_board[x1 - 4][y1 + 4] == side:
                force_move.append((x1 - 2, y1 + 2))
            if len(force_move) != 0:
                force_move_all.append(force_move)
...
def Check_2_in_line(new_board, move, side):  # move 表示检查那个点附近 side表示检查哪一边的，num表示检查连续几个
    ...
    if x - 2 >= 0 and y - 2 >= 0 and new_board[x - 2][y - 2] == side and new_board[x - 1][y - 1] == 0:
        ...
        if x + 1 <= MAX_BOARD - 1 and y + 1 <= MAX_BOARD - 1 and new_board[x + 1][y + 1] == 0:
            force_move.append((x + 1, y + 1))
    if x + 2 <= MAX_BOARD - 1 and y + 2 <= MAX_BOARD - 1 and new_board[x + 2][y + 2] == side and new_board[x + 1][
        y + 1] == 0:
        force_move.append((x + 1, y + 1))
